...
-dialyzer([{[no_opaque, no_return], [prop_commands_with_simple_paths_work_in_any_order/0]}]).
...
command(_State) ->
    elements([
        {call, khepri_machine, put, [?STORE_ID, path(), payload()]},
    ...
    {Entry1, New} =
        case Entries of
            #{Path := #{payload_version := PV} = Entry} ->
                Entry0 =
                    case set_node_payload(Entry, Payload) of
                        Entry -> Entry;
...
add_entry1(Entries, ReversedPath, New) ->
    ...
    {Entry1, New1} =
        case Entries of
            #{
                Path := #{
                    child_list_version := CLV,
                    child_list_length := CLL
                } = Entry
            } when
                New
            ->
