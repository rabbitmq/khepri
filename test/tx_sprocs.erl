    ...
    {setup,
     ...
     [?_assertEqual(
      ...
      ?_assertEqual(
         ...
         khepri:transaction(?FUNCTION_NAME, [tx], [], auto)),
      ?_assertError(
         ?khepri_error(sproc_invalid_as_tx_fun, #{path := [tx]}),
         khepri:transaction(?FUNCTION_NAME, [tx], [], ro)),
      ?_assertEqual(
         ok,
         khepri:transaction(?FUNCTION_NAME, [tx], [], rw))]}.
...
invalid_tx_sproc_test_() ->
    ...
    {setup,
     ...
     [?_assertEqual(
         ok,
         khepri:put(?FUNCTION_NAME, [tx, a], Fun)),
      ?_assertEqual(
         ok,
         khepri:put(?FUNCTION_NAME, [tx, b], not_sproc)),
      ?_assertEqual(
         success,
         khepri:run_sproc(?FUNCTION_NAME, [tx, a], [])),
      ?_assertError(
         ?khepri_error(no_sproc_at_given_path, #{path := [tx, b]}),
         khepri:transaction(?FUNCTION_NAME, [tx, b], [])),
      ?_assertError(
         ?khepri_exception(
            possibly_matching_many_nodes_denied,
            #{path := [tx, ?KHEPRI_WILDCARD_STAR]}),
         khepri:transaction(
           ?FUNCTION_NAME, [tx, ?KHEPRI_WILDCARD_STAR], []))]}.

invalid_tx_sproc_with_old_behaviour_test_() ->
