...
eval_regex_test() ->
    ...
    ?assertEqual(
        {false, {?STAR, {error, {"missing terminating ] for character class", 3}}}},
...
if_has_data_matching_test() ->
    ...
    ?assertEqual(
        ...
        khepri_condition:is_met(
            khepri_condition:compile(#if_has_data{has_data = false}),
            foo,
            #node{payload = #kpayload_data{data = foo}}
        )
    ),
    ?assert(
        khepri_condition:is_met(
...
if_child_list_length_matching_test() ->
    ...
    ?assert(
        khepri_condition:is_met(
            khepri_condition:compile(#if_child_list_length{count = 2}),
            foo,
            #node{child_nodes = #{a => #node{}, b => #node{}}}
        )
    ),
    ?assertEqual(
...
if_not_matching_test() ->
    ?assertEqual(
        ...
        khepri_condition:is_met(
            khepri_condition:compile(#if_not{
                condition =
                    #if_child_list_length{count = 2}
            }),
            foo,
