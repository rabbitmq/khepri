...
unregistering_multi_table_projection_deletes_all_tables_test_() ->
    ProjectFun = fun multi_table_fun/4,
    PathPattern = [stock, wood, <<"oak">>],
    {setup,
     fun() -> test_ra_server_helpers:setup(?FUNCTION_NAME) end,
     ...
     [{inorder,
        [{"Register the projection",
         ...
         {"The ETS tables have the expected type",
          ?_test(
             begin
                 ?assertMatch(bag, ets:info(mtp_all, type)),
                 ?assertMatch(set, ets:info(mtp_latest, type))
             end)},

         {"Unregister the projection",
          ?_assertEqual(
            ok,
            khepri:unregister_projections(?FUNCTION_NAME, [?MODULE]))},
         ...
         {"The ETS tables do not exist anymore",
          ?_test(
             begin
                 ?assertMatch(undefined, ets:info(mtp_all, type)),
                 ?assertMatch(undefined, ets:info(mtp_latest, type))
             end)}]
      }]}.
