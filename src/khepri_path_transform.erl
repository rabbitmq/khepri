...
%%
%% Calls to {@link khepri_path:from_string/1} with a string %% {@link
%% khepri_path:unix_pattern()} are replaced at compile-time with equivalent
%% {@link khepri_path:native_pattern()}s.

-module(khepri_path_transform).

-type form() :: erl_parse:abstract_clause() |
...
-spec map_form(form() | forms(), Fun) -> form() | forms()
    when Fun :: fun((form()) -> form()).
%% @hidden
...
-spec rewrite_path_call(form() | forms()) -> form() | forms().
%% @hidden
...
rewrite_path_call(
  {call,
   _Location,
   {remote, _, {atom, _, khepri_path}, {atom, _, FunctionName}},
   [{bin, Location, _Parts} = BinaryForms]})
  when FunctionName =:= from_string orelse FunctionName =:= from_binary ->
    Binary = erl_parse:normalise(BinaryForms),
    NativePath = khepri_path:from_binary(Binary),
    erl_parse:abstract(NativePath, Location);
