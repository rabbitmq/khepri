...
maybe_request_snapshot(IntState, SideEffects) ->
    case ra_aux:effective_machine_version(IntState) of
        EffectiveMacVer when EffectiveMacVer >= 3 ->
            ...
            %% Depending on whether we know the latest snapshot size (there was
            %% at least one snapshot taken already) or not (no snapshots were
            %% taken yet), we determine the need for a snapshot based on
            %% different factors; see below.
            case SnapshotSize of
                _ when is_integer(SnapshotSize) ->
                    %% Here, we know the latest snapshot size. We compare it to
                    %% the commands added size so far. If the commands added
                    %% size is above, we request a snapshot.
                    CommandsAddedSize = maps:get(
                                          commands_added_size, Metrics,
                                          undefined),
                    ?LOG_DEBUG(
                       "Determine if it is time for a snapshot:~n"
                       "  Latest snapshot size: ~p~n"
                       "  Commands added size:  ~p~n",
                       [SnapshotSize, CommandsAddedSize],
                       #{domain => [khepri, ra_machine]}),
                    if
                        is_integer(CommandsAddedSize) andalso
                        SnapshotSize > 0 andalso
                        CommandsAddedSize >= SnapshotSize ->
                            ?LOG_DEBUG(
                               "Commands added size is above the latest "
                               "snapshot; requesting a snapshot",
                               #{domain => [khepri, ra_machine]}),
                ...
                undefined ->
                    %% We don't have any snapshots yet. We fallback to the
                    %% historic strategy: if the number of applied commands is
                    %% above the configured threshold, we request a snapshot.
                    #config{snapshot_interval = SnapshotInterval} = get_config(
                                                                      State),
                    AppliedCmdCount = maps:get(
                                        applied_command_count, Metrics,
                                        0),

                    ?LOG_DEBUG(
                       "Determine if it is time for the initial snapshot:~n"
                       "  Snapshot interval:      ~p~n"
                       "  Applied commands count: ~p~n",
                       [SnapshotInterval, AppliedCmdCount],
                       #{domain => [khepri, ra_machine]}),
                    if
                        AppliedCmdCount >= SnapshotInterval ->
                            ?LOG_DEBUG(
                               "Applied commands count above the snapshot "
                               "interval; requesting a snapshot",
                               #{domain => [khepri, ra_machine]}),
                            RequestSnapshot = #request_snapshot{},
                            [{append, RequestSnapshot} | SideEffects];
                        true ->
...
apply(
  ...
  State) when MacVer >= 3 ->
    Metrics = get_metrics(State),
    ?LOG_DEBUG(
       "Move release cursor to index ~p to request a snapshot~nMetrics: ~p",
       [RaftIndex, Metrics],
       #{domain => [khepri, ra_machine]}),
