...
%% addition to stored procedures. This comes with two new records: the
...
%% Record sent from {@link khepri_event_handler} to the state machine to
%% acknowledge the execution of a trigger.

-type triggered_v2() :: #triggered_v2{}.
%% Record sent from {@link khepri_event_handler} to the state machine to
%% acknowledge the execution of a trigger.
%%
%% It replaced {@link triggered/0} in the state machine version 4.

-type triggered_list() :: [khepri_machine:triggered() |
                           khepri_machine:triggered_v2()].
%% List of `#triggered{}' or `#triggered_v2{}' records.
...
-record(khepri_machine,
        {config :: khepri_config:machine_config(),
         ...
         emitted_triggers = [] :: khepri_machine:triggered_list(),
...
-spec ack_triggers_execution(StoreId, TriggeredActions) ->
    Ret when
      StoreId :: khepri:store_id(),
      TriggeredActions :: khepri_machine:triggered_list(),
      Ret :: ok | khepri:error().
              ...
              case ReplyTo of
                  {Alias, _Priv}
                    when IsCompatible andalso
                         (is_reference(Alias) orelse Alias =:= gen_server) ->
...
do_apply(Meta, NonVersionedCommand, State)
  when is_record(NonVersionedCommand, put) orelse
       ...
       is_record(NonVersionedCommand, register_trigger) orelse
       is_record(NonVersionedCommand, ack_triggered) orelse
       is_record(NonVersionedCommand, register_projection) orelse
       is_record(NonVersionedCommand, unregister_projection) orelse
       is_record(NonVersionedCommand, unregister_projections) orelse
...
convert_to_uniform_command(
  #register_trigger{id = TriggerId,
    ...
    CommandArgs = #register_trigger_v1{id = TriggerId,
                                       event_filter = EventFilter,
                                       action = Action},
...
convert_to_uniform_command(Command)
  when is_record(Command, put_v) orelse
       ...
       is_record(Command, tx_v) orelse
       is_record(Command, register_trigger_v) orelse
       is_record(Command, ack_triggered_v) orelse
...
return_result_to_appropriate_process(
  State, Result, SideEffects, _Meta, Command) ->
    case get_command_common_args(Command) of
        ...
        #common_v1{reply_to = {gen_server, From}} ->
...
maybe_set_reply_to(Command, #{reply_to := ReplyTo} = Options) ->
    ...
    ?assert(is_reference(element(1, ReplyTo)) orelse element(1, ReplyTo) =:= gen_server),
...
-spec get_emitted_triggers(State) -> EmittedTriggers when
      State :: khepri_machine:state(),
      EmittedTriggers :: khepri_machine:triggered_list().
