...
%% <li>Accept a PID as a `keep_while' condition. The tree node will be deleted
%% as soon as the associated process exits. See {@link
...
register_trigger_v2(
  StoreId, TriggerId, EventFilter, Action, CommandOptions,
  NonCommandOptions) ->
  ...
  IntState) ->
    ...
    {MaybeDownProcs1,
     SideEffects1} = maps:fold(
                       ...
                       end, {MaybeDownProcs, []}, MaybeDownProcs),
    ...
    case ra_aux:effective_machine_version(IntState) of
        ...
        _ ->
            {no_reply, AuxState2, IntState, SideEffects1}
    end;
handle_aux(
  RaState, cast,
...
list_triggered_actions(State, Events, Triggers) ->
    TriggeredActions =
    lists:foldl(
      fun(Event, TA) ->
              ...
              maps:fold(
                fun(TriggerId, TriggerProps, TA1) ->
                        evaluate_trigger(
                          State, Event, TriggerId, TriggerProps, TA1)
                end, TA, Triggers)
      end, [], Events),
    sort_triggered_actions(TriggeredActions).

evaluate_trigger(
  State, #ev_tree{path = Path, change = Change} = Event, TriggerId,
  ...
  TriggeredActions) ->
    ...
    case PathMatches andalso ChangeMatches of
        true ->
            evaluate_action(
              State, Event, TriggerId, Trigger, TriggeredActions);
        false ->
            TriggeredActions
    end;
evaluate_trigger(
  State, #ev_process{pid = Pid} = Event, TriggerId,
  #{event_filter := #evf_process{pid = Pid}} = Trigger,
  TriggeredActions) ->
    %% TODO: Match the exit reason if an optional pattern was provided.
    evaluate_action(
      State, Event, TriggerId, Trigger, TriggeredActions);
evaluate_trigger(_State, _Event, _TriggerId, _Trigger, TriggeredActions) ->
